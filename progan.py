...
class ProgressiveGAN():
    ...
    def checkpoint(self, z, log2_res, step):
        res = 2**log2_res
        prefix = f'res_{res}x{res//2}_{step}'
    ...
    def load_checkpoint(self, path):
        ...
        for i in range(2, max_log2res+1):
            ...
            self.discriminator_blocks[i] = tf.keras.models.load_model(f'{path}/d_{i}', custom_objects={'Conv2D': Conv2D, 'Dense': Dense})
            self.generator_blocks[i] = tf.keras.models.load_model(f'{path}/g_{i}', custom_objects={'PixelNorm': PixelNorm, 'Conv2D': Conv2D, 'Dense': Dense})
    ...
    def train(self, datasets, steps_per_phase = 1000, tick_interval=500):
        ...
        for log2_res in range(self.start_log2_res, self.log2_resolution+1, 1):
            ...
            print(f"Resolution {res}x{res//2}")
